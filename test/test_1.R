...
fit_y_history=fitFlexCoDE(fullHistoryTrain[,2:3],yzTrain[,1],
                      fullHistoryValidation[,2:3],yzValidation[,1],
                      fullHistoryTest[,2:3],yzTest[,1],nIMax = 30,
                      regressionFunction = regressionFunction.SpAM)
fit_y_history$estimatedRisk
print(fit_y_history)
plot(fit_y_history,fullHistoryTest[,2:3],yzTest[,1])

# Fit sparse additive FlexCoDE, Z on full history
fit_z_history=fitFlexCoDE(fullHistoryTrain[,2:3],yzTrain[,2],
                          fullHistoryValidation[,2:3],yzValidation[,2],
                          fullHistoryTest[,2:3],yzTest[,2],nIMax = 30,
                          regressionFunction = regressionFunction.SpAM)
